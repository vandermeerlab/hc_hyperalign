...
NumComponents = 10;
for i = 1:length(TC)
    postcp_TC.left.tc = TC{i}.left.tc(:, max_cp_bin+1:end);
    postcp_TC.right.tc = TC{i}.right.tc(:, max_cp_bin+1:end);
...
for i = 1:100
    ...
    for p_i = 1:length(s_TC)
        s_postcp_TC.left.tc = s_TC{p_i}.left.tc(:, max_cp_bin+1:end);
        s_postcp_TC.right.tc = s_TC{p_i}.right.tc(:, max_cp_bin+1:end);
        s_proj_TC{p_i} = perform_pca(s_postcp_TC, NumComponents);
    end
