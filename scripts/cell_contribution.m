...
max_n_units = max(cellfun(@(x) length(x.left), Q)) - 10;
% ascending: low-firing neurons excluded first
low_first_error = zeros(1, max_n_units);
% descending: high-firing neurons excluded first
high_first_error = zeros(1, max_n_units);
        ...
        if ~isnan(cross_subjs_pairs(sr_i, tar_i))
            ...
            for ex_i = 1:max_excluded_cells
                for or_i = 1:2
                    ...
                    Q_ex.left = Q_sr.left(ex_idx(ex_i+1:end), :);
                    Q_ex.right = Q_sr.right(ex_idx(ex_i+1:end), :);
                    Q_pair = {Q_ex, Q{tar_i}};
                    [actual_dists_mat_ex] = predict_with_L_R([], Q_pair);
                    delta_error(or_i, ex_i) = (actual_dists_mat_ex(1, 2) - actual_error) / actual_error;
                end
            end
            if size(low_first_error, 1) == 1
                low_first_error = delta_error(1, :);
                high_first_error = delta_error(2, :);
            else
...
mean_error = zeros(2, max_n_units);
sem_error = zeros(2, max_n_units);

% Only calculate mean and sem for non-zero elements.
for i = 1:max_n_units
    mean_error(1, i) = mean(nonzeros(lf_error_percent(:, i)));
    sem_error(1, i) = std(nonzeros(lf_error_percent(:, i))) / sqrt(n_subjs*(n_subjs-1));
    mean_error(2, i) = mean(nonzeros(hf_error_percent(:, i)));
    sem_error(2, i) = std(nonzeros(hf_error_percent(:, i))) / sqrt(n_subjs*(n_subjs-1));
end

errorbar(1:max_n_units, mean_error(1, :), sem_error(1, :));
hold on;
errorbar(1:max_n_units, mean_error(2, :), sem_error(2, :));
xlabel('Number of cells excluded'); ylabel('Delta prediction error (pencentage)');
