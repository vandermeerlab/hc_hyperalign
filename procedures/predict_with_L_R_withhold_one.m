function [actual_dists_mat, id_dists_mat, predicted_Q_mat] = predict_with_L_R_withhold_one(cfg_in, Q)
    ...
    % This version uses data of one target R trial for hyperalignment and
    % predicts the rest withholding target R data.
    ...
    for sr_i = 1:length(Q)
        for tar_i = 1:length(Q)
            if sr_i ~= tar_i
                ...
                ex_Q = Q;
                ex_Q{tar_i}.right = Q{tar_i}.right_c;
                % PCA
                ex_proj_Q = proj_Q;
                ex_eigvecs = eigvecs;
